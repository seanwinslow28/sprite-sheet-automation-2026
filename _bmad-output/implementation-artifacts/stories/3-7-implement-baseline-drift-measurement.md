...
- [ ] **Task 1: Create baseline drift detector** (AC: #1, #2)
  - [ ] 1.1: Create `src/core/metrics/baseline-drift-detector.ts`
  - [ ] 1.2: Implement `measureBaselineDrift(candidatePath: string, anchorAnalysis: AnchorAnalysis): Result<BaselineDriftResult, SystemError>`
  - [ ] 1.3: Return `BaselineDriftResult` with: `drift_pixels`, `candidate_baseline`, `anchor_baseline`, `direction`
  - [ ] 1.4: Reuse visible bounds logic from anchor analyzer (Story 2.7)

- [ ] **Task 2: Detect candidate baseline** (AC: #1)
  - [ ] 2.1: Load candidate image with Sharp
  - [ ] 2.2: Get raw RGBA pixel buffer
  - [ ] 2.3: Scan rows from bottom to top
  - [ ] 2.4: Find last row with any opaque pixel (alpha >= 128)
  - [ ] 2.5: This row Y-coordinate is the candidate baseline

- [ ] **Task 3: Calculate drift offset** (AC: #2)
  - [ ] 3.1: Get anchor baseline from `anchorAnalysis.results.baselineY`
  - [ ] 3.2: Calculate drift: `candidateBaseline - anchorBaseline`
  - [ ] 3.3: Positive drift = sprite is lower (sinking)
  - [ ] 3.4: Negative drift = sprite is higher (floating)
  - [ ] 3.5: Calculate absolute drift for threshold comparison

- [ ] **Task 4: Implement threshold evaluation** (AC: #3)
  - [ ] 4.1: Read `auditor.thresholds.baseline_drift_max` (default: 4 pixels)
  - [ ] 4.2: If |drift| > threshold, return `SF04_BASELINE_DRIFT` soft fail
  - [ ] 4.3: Include drift direction and magnitude in failure details
  - [ ] 4.4: Note: 0 drift is ideal, but small drift is acceptable

- [ ] **Task 5: Implement logging** (AC: #4)
  - [ ] 5.1: Log drift value with sign (+/-)
  - [ ] 5.2: Log direction (floating/sinking/aligned)
  - [ ] 5.3: Log anchor vs candidate baseline Y-coordinates
  - [ ] 5.4: Include in frame metrics JSON

- [ ] **Task 6: Handle edge cases** (AC: all)
  - [ ] 6.1: Handle fully transparent frame (error)
  - [ ] 6.2: Handle frame where baseline is at image edge
  - [ ] 6.3: Handle anchor analysis missing (error)

- [ ] **Task 7: Write tests** (AC: all)
  - [ ] 7.1: Test aligned sprites return drift 0
  - [ ] 7.2: Test floating sprite returns negative drift
  - [ ] 7.3: Test sinking sprite returns positive drift
  - [ ] 7.4: Test threshold triggers soft fail
  - [ ] 7.5: Test fully transparent frame returns error
